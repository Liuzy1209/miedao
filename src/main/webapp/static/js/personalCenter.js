$(function() {
	$.ajax({
		...
		success: function (data) {
			if (data.data != null) {
				var user = data.data;
				console.log("===="+user.u_pic)
				var nickname = document.getElementById("nickname");
				nickname.innerText=user.u_name;
				var pic = document.getElementById("u-pic");
				pic.setAttribute('style', 'background: url(http://localhost:8080/'+user.u_pic+') no-repeat center center;background-size:cover;');
				console.log('url(http://localhost:8080/'+user.u_pic+')')
			} else {
